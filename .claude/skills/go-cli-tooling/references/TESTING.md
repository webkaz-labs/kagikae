...
- **Have the extractor refuse a block whose checks are only comments, before it runs
  ...
  than `[A-Za-z]+:`, which was measured flagging a legitimate narration comment. Both
  measurements named there are over-refusals; the escapes behind the case, vocabulary and
  pre-colon rules were measured in the tool this comes from and are recorded with it.
