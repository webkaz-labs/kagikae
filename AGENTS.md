...
| [docs/ACCEPTANCE.md](docs/ACCEPTANCE.md) | before cutting a release — what a release owes on a real machine, and **§ Open gates**, which it still owes. Also **when claude asks for a login it did not ask for before, or a deadline kae reported passes and it keeps serving** — read what that section says to read before logging back in. It is also where a gate's result is recorded. Nothing here runs from `mise run check` |
