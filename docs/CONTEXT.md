...
| **breadcrumb** | the file recording which absolute path a pin-id hashes from; [DATA-MODEL.md](DATA-MODEL.md) § Directory Layout (XDG) calls it the bound-directory record | Read when the question is "does this pin still point anywhere". Distinct from the fragment below, and the pair is asymmetric: `kae unpin` removes the fragment and never the breadcrumb |
| **fragment** | the kae-owned mise config file inside a bound directory. What counts as bound comes from the fragment, not from the store tree ([CLI.md](CLI.md) § kae pin) | |
