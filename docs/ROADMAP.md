...
- **`kae relogin`'s success line reports a login for a store somebody *else* refreshed**
  — implemented for the v0.18.2 target by reporting the observed credential capture.
  [CLI.md](CLI.md) § kae relogin Semantics owns the wording and observation gates;
  `TestReloginReportsCaptureWhenASiblingRefreshesDuringFailedFlow` covers a sibling
  refreshing the shared store during a failed login flow. Establishing which process
  changed the store would require evidence beyond those gates; this release adds no
  causal-observation interface.
...
- **agy's file store on macOS** (recorded gap, 2026-07-31; re-read on 1.1.23,
  2026-09-04): agy skips the keychain under ssh/wsl/container/dbus detection, on a
