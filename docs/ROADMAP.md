...
2026-08-16 across the wider 2.1.220 → 2.1.233 span, six of claude's moved and
opencode's `auth.json` moved by one, so the stability is a property of adjacent
releases rather than of the literals. That is the check working as designed — a moved
count is an instruction to work the tool's rows, which is what
[VALIDATION.md](VALIDATION.md) § Upstream Behaviour Assumptions records the result of
— but it does mean the cost of a re-measure scales with how long a version sits. The remaining two, in the order each pays for itself:
