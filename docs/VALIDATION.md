...
| agy | **the macOS keychain is conditional, not the platform's answer.** agy's auth package pairs a keyring store with a file store: `shouldBypassKeyring` sits next to an ssh / wsl / container / **dbus** detector, keyring operations carry a timeout, a failure falls back to the file ("Keyring SaveToken timed out after %v, falling back to file storage", plus load, remove, project-and-region and user-tier variants), and since 1.1.12 a timeout is **persisted**: "a keyring timeout was recorded within the last %v", written under a `keyring-marker-` name, so a bypass can outlive the run that caused it. So a remote-shell session on a Mac uses the file store, and kae's keychain switch does not reach it. agy **shells out to `/usr/bin/security`** (a Go keyring library, `go-keyring-base64:` / `go-keyring-encoded:` payload prefixes) | Read the binary, no login: `grep -Foa -- '<literal>' <binary> \| wc -l`, then extract printable runs (`re.finditer(rb'[\x20-\x7e]{6,}')` — `strings` truncates but works too) and grep those for the type names. **Anchor neither on the package nor on the type name**, because between 1.0.10 and 1.1.12 both moved, and the counts are what settled it: `shouldBypassKeyring`, the detectors and a keyring/file/chooser trio left `jetski/cli/backend/auth/auth` for `jetski/language_server/code_assist_client/codeassistclient`, where they are `KeyringTokenStorage` / `fileTokenStorage` / `compositeTokenStorage` (the chooser, with `isUseFileStatic`, `memoizeKeyringTimeout`, `recordKeyringError`) and `keyring_detector_{ssh,wsl,container,dbus}.go`. The move was **not** wholesale, which is the part a "renamed with it" reading gets wrong: `cliTokenStorage` went 13 → **0** (the CLI layer's keyring store is gone) while `cliFileTokenStorage` went 5 → **11** and is still under the old package. The detector inputs are the literals `SSH_TTY`, `SSH_CONNECTION`, `SSH_CLIENT`, `WSL_DISTRO_NAME`, `WSL_INTEROP`, `/.dockerenv`, `/proc/1/cgroup`, each occurring once in 1.0.10, 1.1.12 and 1.1.13 alike; the dbus one adds none kae can see (`DBUS_SESSION_BUS_ADDRESS` and `DBUS_SYSTEM_BUS_ADDRESS` occur **0** times in all three). **The `security` PATH shim does not reach agy**, which this table claimed it did. What carries that is one literal: the binary spells `/usr/bin/security` out (once in each of 1.0.10, 1.1.12 and 1.1.13), an absolute path no `PATH` entry precedes, and the only keychain-exec library in it is zalando/go-keyring (`keyring_darwin.go`, once in each; `SecItemAdd` and `SecItemCopyMatching` **0** in each). A shim run on 1.1.12 left the log empty, which is **consistent with that and discriminates nothing** — this page says an empty log cannot be told from a tool that never asked, and that run's own agy log says `You are not logged into Antigravity`, which the never-reached-the-keyring hypothesis predicts too. agy has no positive discriminator the way codex does. Live corroboration without a login: `~/.gemini/antigravity-cli/log/*.log` records `ChainedAuth: authenticated via keyring (effective: keyring)` per run — an `effective:` other than `keyring` on macOS is this row breaking. **That corroboration is stale as of 2026-08-17 and reading it as current is the trap here**: the newest such line on the measuring machine is dated 2026-06-22 and the `gemini`/`antigravity` item's `mdat` is the same day, both from 1.0.10, so nothing has observed 1.1.12 or 1.1.13 choosing a store. **Still unmeasured: the fallback file's path.** What *is* measured is that it is none of the three `credentialFiles` names: `credentials.enc` and `oauth_creds.json` occur **0** times in all three builds, and `credentials.json`'s two hits are both substrings (`application_default_credentials.json`, `client_credentials.json`) rather than a filename. That is why kae warns instead of declaring an artifact. ⚠️ **Probing agy upgrades it.** The temp-`HOME` run above sits in the same minute (00:47, 2026-08-17) as a new binary appearing in agy's mise install directory, across which `agy --version` moved 1.1.12 → 1.1.13; the previous build survives beside it under an `agy.<digits>.old` symlink. So the directory name is a version only until agy replaces what is in it, and a re-measure has to read `--version`, never the path | 1.1.13 (binary read, 2026-08-17, taken on 1.1.12 and re-run on 1.1.13 with every count and `codeassistclient` symbol identical; live logs 1.0.10, 2026-06-22) |
