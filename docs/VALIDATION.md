...
derivations rather than claims. The precedence is one offset-read window on 2.1.233,
...
copilot's is the root its launcher searched first at 1.0.79, rather than the
documented config dir, and the `darwin-x64` segment is one machine's architecture. And the
...
**Two of these paths were repointed on 2026-08-17 because they had been reading
...
notes and moves when upstream writes about a variable: of `COPILOT_HOME`'s 61, five
are there, and one of `COPILOT_GITHUB_TOKEN`'s 29. That is the noise this section
rejects rows for, and they are kept anyway because narrowing the artifact to one file
is what hid the move in the first place — those literals are spread over thirteen
files. Read a change in those two as "look", not as "the auth code moved".
