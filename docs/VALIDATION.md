...
| agy | **the macOS keychain is conditional, not the platform's answer.** agy's auth package pairs a keyring store with a file store: `shouldBypassKeyring` sits next to an ssh / wsl / container / **dbus** detector, keyring operations carry a timeout, a failure falls back to the file ("Keyring SaveToken timed out after %v, falling back to file storage", plus load, remove, project-and-region and user-tier variants), and since 1.1.12 a timeout is **persisted**: "a keyring timeout was recorded within the last %v", written under a `keyring-marker-` name, so a bypass can outlive the run that caused it. So a remote-shell session on a Mac uses the file store, and kae's keychain switch does not reach it. agy **shells out to `/usr/bin/security`** (a Go keyring library, `go-keyring-base64:` / `go-keyring-encoded:` payload prefixes) | Read the binary, no login: `grep -Foa -- 'falling back to file' <binary> \| wc -l` for the fallback strings, then extract printable runs (`re.finditer(rb'[\x20-\x7e]{6,}')` — `strings` truncates but works too) and grep them for `codeassistclient\.` to list the store types. **The package moved between 1.0.10 and 1.1.12** and the type names went with it, so grepping the old path finds nothing and reads as removal: `jetski/cli/backend/auth/auth` held `cliTokenStorage` (keyring) / `cliFileTokenStorage` (file) / `shouldBypassKeyring` / `containerDetector` / `sshDetector` / `wslDetector`, and `jetski/language_server/code_assist_client/codeassistclient` now holds `KeyringTokenStorage` / `fileTokenStorage` / `compositeTokenStorage` (the chooser, with `isUseFileStatic`, `memoizeKeyringTimeout`, `recordKeyringError`) / `shouldBypassKeyring` / `keyring_detector_{ssh,wsl,container,dbus}.go`. `cliTokenStorage` occurs **0** times in 1.1.12 for that reason and not because the keyring store went away. The detector inputs are the literals `SSH_TTY`, `SSH_CONNECTION`, `SSH_CLIENT`, `WSL_DISTRO_NAME`, `WSL_INTEROP`, `/.dockerenv`, `/proc/1/cgroup`; the dbus one adds none kae can see (`DBUS_SESSION_BUS_ADDRESS` and `DBUS_SYSTEM_BUS_ADDRESS` occur **0** times in 1.1.12). **The `security` PATH shim does not reach agy**, which this table claimed it did: zalando's `keyring_darwin.go` carries the literal `/usr/bin/security` (once in each of 1.0.10 and 1.1.12), an absolute path no `PATH` entry precedes — and a shim run on 1.1.12 (`env -i HOME=<temp> PATH=<shim>:/usr/bin:/bin agy models`) left the shim log empty. Live corroboration without a login: `~/.gemini/antigravity-cli/log/*.log` records `ChainedAuth: authenticated via keyring (effective: keyring)` per run — an `effective:` other than `keyring` on macOS is this row breaking. **That corroboration is stale as of 2026-08-17 and reading it as current is the trap here**: the newest such line on the measuring machine is dated 2026-06-22 and the `gemini`/`antigravity` item's `mdat` is the same day, both from 1.0.10, so nothing observed 1.1.12 choosing a store. **Still unmeasured: the fallback file's path**, which is why kae warns instead of declaring an artifact | 1.1.12 (binary read, 2026-08-17; live logs 1.0.10, 2026-06-22) |
