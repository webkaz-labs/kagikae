...
text, and two of those say which *half* of themselves was re-measured. Everything
...
derivations rather than claims. The precedence is one offset-read window on 2.1.246,
...
- `claude.KeychainService` is `"Claude Code-credentials"`, and that string occurs
  **0** times — re-confirmed on 2.1.246, 2026-09-04, as it was on 2.1.220. Upstream
...
|---|---|---|
| claude | `realpath(command -v claude)` (one Mach-O, JS inline) | `2.1.246` |
...
| Tool | Literal | Count |
|---|---|---|
| claude | `-credentials` | 27 |
| claude | `claude-code-user` | 3 |
| claude | `CLAUDE_CONFIG_DIR` | 51 |
| claude | `CLAUDE_SECURESTORAGE_CONFIG_DIR` | 10 |
| claude | `CLAUDE_CODE_CUSTOM_OAUTH_URL` | 9 |
| claude | `claudeAiOauth` | 27 |
