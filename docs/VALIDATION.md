...
**Every path is under `codex-rs/`** — the prefix was missing here until 2026-08-16,
and without it the command 404s at every tag, so it fails the same way whether or not
the file moved. What to fetch, and where each actually lives, since two of them are
not where their names suggest:

| What | Path under `codex-rs/` |
|---|---|
| the keyring item's store key and codex's own scoped delete | `login/src/auth/storage.rs` |
| `CODEX_HOME` canonicalization | `utils/home-dir/src/lib.rs` |
| the store-mode resolution | `core/src/config/auth_keyring.rs` |
| `AuthCredentialsStoreMode` | `config/src/types.rs` |
| `cli_auth_credentials_store` | `config/src/config_toml.rs` |
| the `secret_auth_storage` `FeatureSpec` | `features/src/lib.rs` — **not** `features/src/feature_configs.rs` |

Run for 0.145.0 → 0.146.0 on 2026-08-04 and again for 0.146.0 → 0.147.0 on
2026-08-16, and on both runs every row of that table came back either byte-identical
or additive-only. What each one showed at 0.147.0 is on the assumption rows
themselves, which is where a reader needs it.
