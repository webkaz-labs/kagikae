...
| copilot | **the config directory is a rule**: `--config-dir` (deprecated, hidden) → `COPILOT_HOME` → `~/.copilot`, where `COPILOT_HOME` is the directory *itself* and is used verbatim (no normalization, no absolute-path check). Setting it also skips copilot's one-way `$XDG_CONFIG_HOME/.copilot` → `~/.copilot` migration. The auth write is always `config.json`; the bare `config` file in the same directory is a fallback of the settings-migration loader only. A **relative** value is resolved against the process's own cwd by both sides, so kae warns rather than pretending to know copilot's | The **installed CLI is not the binary on `PATH`**: that one is a launcher which loads an `app.js` out of the first `pkg` directory it finds, so a version manager can pin an older launcher while `copilot --version` reports the newer package — verify against the version `copilot --version` prints. **Where that `pkg` is has moved**, and reading the old path is how a stale bundle gets measured: at 1.0.79 the launcher builds its search list as `$COPILOT_PKG_CACHE_HOME/pkg`, `$COPILOT_CACHE_HOME/pkg`, `li()/pkg`, `Lr()/copilot/pkg`, `$COPILOT_HOME/pkg`, then `homedir()/.copilot/pkg` **last** — where `li()` is, read from the launcher, `join(homedir(),"Library","Caches","copilot")` on darwin, `(LOCALAPPDATA ?? homedir()/.cache)/copilot` on win32 and `Lr()/copilot` otherwise, and `Lr()` is `XDG_CACHE_HOME ?? homedir()/.cache`. So on macOS the live bundle is `~/Library/Caches/copilot/pkg/darwin-x64/<version>/`, a directory rather than one file, and `~/.copilot/pkg/universal/<version>/app.js` is where older packages stay behind. Grep the launcher for `Library/Caches` and you get **0** — it is three arguments to `join`, so search for `COPILOT_PKG_CACHE_HOME` and read the function instead. The launcher also updates the package itself, which `--no-auto-update`, `--prefer-version <v>` and `COPILOT_AUTO_UPDATE=false` each turn off; use one of those before measuring, or the bundle can move mid-measurement the way agy's binary does. The auth keys moved with it: `lastLoggedInUser` and `loggedInUsers` occur **0** times in 1.0.79's `app.js` and live in `prebuilds/darwin-x64/runtime.node` (a serde-tagged native module), so grep the tree, not `app.js`. `grep -o 'COPILOT_HOME[^,;)]\{0,110\}' app.js` still shows the resolver (`t?.configDir ?? process.env.COPILOT_HOME ?? join(homedir(),".copilot")`, now also reached through a native `resolveCopilotHome(configDir, COPILOT_HOME, homedir())`), the loader's file list (`config.json`, then `config`), and copilot's own help text ("override the directory where configuration and state files are stored"). The `$XDG_*` migration `COPILOT_HOME` skips is **two** variables, not one — `preAction` runs `process.env.COPILOT_HOME \|\| _On()` and `_On()` migrates both `$XDG_STATE_HOME/.copilot` and `$XDG_CONFIG_HOME/.copilot` into `homedir()/.copilot` (never into `COPILOT_HOME`), leaving a symlink behind and skipping any name that already exists at the destination. Behavioural confirmation, **cheaper than the recipe this row used to carry**: the pkg symlink it prescribed is unnecessary now that the launcher finds the bundle in the OS cache regardless of `COPILOT_HOME`, so it is just `mcp-config` naming a probe server in `$T` and `env -i HOME=$HOME COPILOT_HOME=$T copilot mcp list --json`, which lists it. For the relative half, run that from two directories: with `COPILOT_HOME=relhome` the probe is found from the parent of `relhome` and **not** from inside it | 1.0.79 (source + behaviour, 2026-08-17) |
