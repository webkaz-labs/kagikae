...
the unit tests over the darwin sim and the optional real-machine checks in
...
grep -q 'the binding is in place' "$HOME/F.err"
                                       ...
                                       #   the bound directory: a worktree can be
...
Both belong in real-machine acceptance rather than here.
...
- `TestMiseHookBlockUsesCurrentShellSemantics` — each shell's global completion
  hook names that shell and uses mise's current-shell `script` form. This is the
  semantic distinction a TOML parse cannot prove: omitting `shell` makes mise spawn
  its default inline shell, where a defined completion function cannot reach the
  caller and zsh process substitution does not parse under `sh`.
- `TestCompletionRefreshMigratesExactLegacyMiseHook` and
  `TestCompletionRefreshLeavesNonLegacyMiseBlockUntouched` — refresh replaces the
  exact old kae-owned block while preserving surrounding TOML, but does not infer
  that a foreign or edited block is safe to rewrite.

### Completion real-machine smoke (required before release)

Run `bash scripts/smoke-run.sh '## Shell completion for'`. Bash and zsh are
required; fish remains best-effort. This exercises refreshed registrations and
the real mise current-shell hook, using fresh shell processes and the built
binary's completion backend. It does not simulate a physical TAB key.

```bash
cp scripts/release-smoke/mise-hook.zsh "$HOME/check-mise-hook"
mkdir -p "$HOME/bin" "$XDG_CONFIG_HOME/zsh/completions" "$XDG_DATA_HOME/bash-completion/completions" "$XDG_CONFIG_HOME/kagikae"
go build -o "$HOME/bin/kae" .
export PATH="$HOME/bin:$PATH"
printf '%s\n' '# stale registered completion' > "$XDG_CONFIG_HOME/zsh/completions/_kae"
printf '%s\n' '# stale registered completion' > "$XDG_DATA_HOME/bash-completion/completions/kae"
kae completion --refresh
mkdir -p "$XDG_DATA_HOME/kagikae/accounts/claude/main"
printf 'version = 1\ntool = "claude"\naccount = "main"\n' > "$XDG_DATA_HOME/kagikae/accounts/claude/main/account.toml"
cp scripts/release-smoke/completion.bash "$HOME/check-bash"
bash --noprofile --norc "$HOME/check-bash"
cp scripts/release-smoke/completion.zsh "$HOME/check-zsh"
...
Run `bash scripts/smoke-run.sh '## Switching a per-account credential store'`.
The block uses synthetic accounts, the file driver and file snapshot backend;
its upstream fixture accepts only `--version`. Credential-program interceptors
must remain unused. The sequence checks shared credential paths, migration and
repair, reference-counted purge with its account-wide message, and shared-mode
rebind through the built CLI. Real-keychain acceptance stays in
[ACCEPTANCE.md](ACCEPTANCE.md).

```bash
test "$KAE_CLAUDE_DRIVER" = file
mkdir -p "$HOME/.claude" "$XDG_CONFIG_HOME/kagikae" "$HOME/shims" "$HOME/main-app" "$HOME/side-project"
go build -o "$HOME/kae" .
go build -o "$HOME/storecheck" ./scripts/release-smoke/storecheck
KAE_STORE_SMOKE_BIN="$HOME/kae"
printf 'default_profile = "main"\n[security]\nsecret_backend = "file"\n[profiles.main]\naccounts = { claude = "main" }\n[tools.claude]\nenabled = true\n[tools.codex]\nenabled = false\n[tools.agy]\nenabled = false\n[tools.opencode]\nenabled = false\n[tools.cursor]\nenabled = false\n[tools.copilot]\nenabled = false\n' > "$XDG_CONFIG_HOME/kagikae/config.toml"
printf '#!/bin/sh\nprintf "security intercepted\\n" >> "$HOME/credential-calls"\nprintf "could not be found\\n" >&2\nexit 44\n' > "$HOME/shims/security"
printf '#!/bin/sh\nprintf "secret-tool intercepted\\n" >> "$HOME/credential-calls"\nexit 1\n' > "$HOME/shims/secret-tool"
printf '#!/bin/sh\nif [ "$#" -eq 1 ] && [ "$1" = --version ]; then printf "2.1.261 (Claude Code)\\n"; else exit 77; fi\n' > "$HOME/shims/claude"
chmod 700 "$HOME/shims/security" "$HOME/shims/secret-tool" "$HOME/shims/claude"
export PATH="$HOME/shims:$PATH"
test "$(command -v security)" = "$HOME/shims/security"
test "$(command -v secret-tool)" = "$HOME/shims/secret-tool"
test "$(command -v claude)" = "$HOME/shims/claude"
DEADLINE=$((($(date +%s) + 86400) * 1000))
printf '{"claudeAiOauth":{"accessToken":"sk-ant-oat01-MAIN-SYNTHETIC-aaaa","refreshToken":"main-synthetic-refresh","expiresAt":%s,"subscriptionType":"max"}}\n' "$DEADLINE" > "$HOME/.claude/.credentials.json"
printf '{"oauthAccount":{"accountUuid":"main-uuid","emailAddress":"you@example.com","organizationUuid":"main-org"},"projects":{}}\n' > "$HOME/.claude.json"
"$KAE_STORE_SMOKE_BIN" add --no-login claude main
DEADLINE=$((($(date +%s) + 86400) * 1000))
printf '{"claudeAiOauth":{"accessToken":"sk-ant-oat01-SIDE-SYNTHETIC-bbbb","refreshToken":"side-synthetic-refresh","expiresAt":%s,"subscriptionType":"max"}}\n' "$DEADLINE" > "$HOME/.claude/.credentials.json"
printf '{"oauthAccount":{"accountUuid":"side-uuid","emailAddress":"you@example.com","organizationUuid":"side-org"},"projects":{}}\n' > "$HOME/.claude.json"
"$KAE_STORE_SMOKE_BIN" add --no-login claude side
FIRST=$(cd "$HOME/main-app" && pwd -P)
SECOND=$(cd "$HOME/side-project" && pwd -P)
FRAGMENT=.config/mise/conf.d/kagikae.toml
fragvar() { "$HOME/storecheck" fragment "$1" "$2"; }
cd "$FIRST"
"$KAE_STORE_SMOKE_BIN" pin main
FIRST_CONFIG=$(fragvar "$FIRST/$FRAGMENT" CLAUDE_CONFIG_DIR)
SHARED_CRED=$(fragvar "$FIRST/$FRAGMENT" CLAUDE_SECURESTORAGE_CONFIG_DIR)
cd "$SECOND"
"$KAE_STORE_SMOKE_BIN" pin main
SECOND_CONFIG=$(fragvar "$SECOND/$FRAGMENT" CLAUDE_CONFIG_DIR)
test "$FIRST_CONFIG" != "$SECOND_CONFIG"
test "$(fragvar "$SECOND/$FRAGMENT" CLAUDE_SECURESTORAGE_CONFIG_DIR)" = "$SHARED_CRED"
test "$SHARED_CRED" = "$XDG_DATA_HOME/kagikae/credstore/claude/main"
"$HOME/storecheck" credential "$SHARED_CRED" sk-ant-oat01-MAIN-SYNTHETIC-aaaa
"$KAE_STORE_SMOKE_BIN" doctor --json > "$HOME/store-normal.json"
"$HOME/storecheck" normal "$HOME/store-normal.json"
# Deliberately corrupt only the synthetic fragment to reproduce the old unsplit shape.
sed '/^CLAUDE_SECURESTORAGE_CONFIG_DIR = /d' "$FIRST/$FRAGMENT" > "$HOME/unsplit-fragment"
mv "$HOME/unsplit-fragment" "$FIRST/$FRAGMENT"
"$KAE_STORE_SMOKE_BIN" doctor --json > "$HOME/store-unsplit.json"
"$HOME/storecheck" unsplit "$HOME/store-unsplit.json" "$FIRST"
cd "$FIRST"
"$KAE_STORE_SMOKE_BIN" pin main
test "$(fragvar "$FIRST/$FRAGMENT" CLAUDE_SECURESTORAGE_CONFIG_DIR)" = "$SHARED_CRED"
...
"$KAE_STORE_SMOKE_BIN" pin claude side
test "$(fragvar "$SECOND/$FRAGMENT" CLAUDE_CONFIG_DIR)" = "$BEFORE_REBIND"
SIDE_CRED=$(fragvar "$SECOND/$FRAGMENT" CLAUDE_SECURESTORAGE_CONFIG_DIR)
test "$SIDE_CRED" != "$SHARED_CRED"
test "$SIDE_CRED" = "$XDG_DATA_HOME/kagikae/credstore/claude/side"
"$HOME/storecheck" credential "$SIDE_CRED" sk-ant-oat01-SIDE-SYNTHETIC-bbbb
test ! -s "$HOME/credential-calls"
```
...
right. codex is the only such tool today. Run it as
`gh api -H "Accept: application/vnd.github.raw"
"repos/openai/codex/contents/<path>?ref=rust-v<version>"` for each tag, then `diff`,
with `<path>` copied whole from this table — **this table is the one copy of where
codex keeps what kae reads**, so a file that moves upstream is corrected here and
cited from elsewhere rather than re-listed:

| What | `<path>` |
|---|---|
| the keyring item's store key and codex's own scoped delete | `codex-rs/login/src/auth/storage.rs` |
| `CODEX_HOME` canonicalization | `codex-rs/utils/home-dir/src/lib.rs` |
| the store-mode resolution | `codex-rs/core/src/config/auth_keyring.rs` |
| `AuthCredentialsStoreMode` | `codex-rs/config/src/types.rs` |
| `cli_auth_credentials_store` | `codex-rs/config/src/config_toml.rs` |
| the `secret_auth_storage` `FeatureSpec` | `codex-rs/features/src/lib.rs` — **not** `codex-rs/features/src/feature_configs.rs` |

The paths are whole on purpose. This paragraph carried them without their
`codex-rs/` prefix until 2026-08-16, which 404s at **every** tag — the same failure a
moved file gives, so it cannot be told from one — and that drift is the argument for
the table: the assumption rows below had the prefix all along, and nothing compared
the two copies. **A 404 on a path copied whole is therefore upstream having moved it**:
find the declaration by name in the repository and correct the table in that commit.
What each run showed is on the assumption rows themselves, which is where a reader
needs it, and each says which half of itself the reading covers.
**Compare the declaration, not the file, when the file also changed** — at 0.147.0
`features/src/lib.rs` gained unrelated features while the `SecretAuthStorage` spec did
not move a byte — and **check the hit count is non-zero on both sides**, since a
grep-versus-grep comparison of two empty results reports "identical". That last one is
not hypothetical: grepping `feature_configs.rs` for `secret_auth_storage` returns zero
...
in a function whose neighbours are runner file-walking rather than credentials:
`let r = process.env.SELF_HOSTED_RUNNER_HOST_CONFIG_DIR || process.env.CLAUDE_CONFIG_DIR
|| join(homedir(), ".claude")` — so **one** site was seen and no claim is made about
...
| agy | **the macOS keychain is conditional, not the platform's answer.** agy's auth package pairs a keyring store with a file store: `shouldBypassKeyring` sits next to an ssh / wsl / container / **dbus** detector, keyring operations carry a timeout, a failure falls back to the file ("Keyring SaveToken timed out after %v, falling back to file storage", plus load, remove, project-and-region and user-tier variants), and a timeout is now **persisted**: "a keyring timeout was recorded within the last %v", written under a `keyring-marker-` name, so a bypass can outlive the run that caused it. That last one is newer than 1.0.10 and no more precise than that — `keyring-marker-`, `DefaultKeyringMarkerPath` and `keyringRecentlyUnavailable` each count 0 in 1.0.10 and 1 in 1.1.12, 1.1.13, 1.1.22 and 1.1.23, and no build between 1.0.10 and 1.1.12 was read, so which release introduced it is unknown rather than 1.1.12. So a remote-shell session on a Mac uses the file store, and kae's keychain switch does not reach it. agy **shells out to `/usr/bin/security`** (a Go keyring library, `go-keyring-base64:` / `go-keyring-encoded:` payload prefixes) | Read the binary, no login. **Resolve `command -v agy` in the environment being audited.** On 2026-09-04 the interactive shell selected retained 1.1.22 while `mise exec` and `mise run audit` selected current 1.1.23; a lookup outside the task would have measured the stale build without complaint. Count literals by § Re-measuring below, and get the type names by extracting printable runs (`re.finditer(rb'[\x20-\x7e]{6,}')`; `strings` truncates but works too). **Anchor a version diff on neither the package nor the type name** — both moved between 1.0.10 and 1.1.12, and [measuring.md](../.claude/skills/upstream-auth-drift/references/measuring.md) owns why and what to do instead. What that move was, as evidence for this row: `shouldBypassKeyring`, the detectors and a keyring/file/chooser trio left `jetski/cli/backend/auth/auth` for `jetski/language_server/code_assist_client/codeassistclient`, where they are `KeyringTokenStorage` / `fileTokenStorage` / `compositeTokenStorage` (the chooser, with `isUseFileStatic`, `memoizeKeyringTimeout`, `recordKeyringError`) and `keyring_detector_{ssh,wsl,container,dbus}.go` — and it was **not** wholesale, since `cliTokenStorage` went 13 → **0** while `cliFileTokenStorage` went 5 → **11** and stayed where it was. The detector inputs are the literals `SSH_TTY`, `SSH_CONNECTION`, `SSH_CLIENT`, `WSL_DISTRO_NAME`, `WSL_INTEROP`, `/.dockerenv`, `/proc/1/cgroup`. **The `security` PATH shim does not reach agy**, which this table claimed it did, so do not try to measure this row with one. The derivation and its evidence live once, in [measuring.md](../.claude/skills/upstream-auth-drift/references/measuring.md) § Does a `PATH` shim reach this tool? — including why the empty log a shim run does produce settles nothing, and that as of 2026-08-17 no positive discriminator like codex's had been found for agy. Live corroboration without a login: `~/.gemini/antigravity-cli/log/*.log` records `ChainedAuth: authenticated via keyring (effective: keyring)` per run — an `effective:` other than `keyring` on macOS is this row breaking. **That corroboration is stale as of 2026-08-17 and reading it as current is the trap here**: the newest such line on the measuring machine is dated 2026-06-22 and the `gemini`/`antigravity` item's `mdat` is the same day, both from 1.0.10, so nothing has observed a post-1.0.10 build choosing a store. **Still unmeasured: the fallback file's path**, though it is at least none of the `credentialFiles` names: read through 2026-09-04 on 1.0.10, 1.1.12, 1.1.13, 1.1.22 and 1.1.23, `credentials.enc` and `oauth_creds.json` occur **0** times in each (recorded as rows below) and `credentials.json` never stands alone in any measured build, its only hits being inside `application_default_credentials.json` and `client_credentials.json` — which is why that one is deliberately not a row. That is why kae warns instead of declaring an artifact. ⚠️ **Probing agy upgrades it, so copy the binary out before running it** — the trap and what it cost are in [measuring.md](../.claude/skills/upstream-auth-drift/references/measuring.md). Re-read on 2026-09-06: the installed bytes matched the executable in the official 1.1.24 release archive. Compared with retained 1.1.23, the recorded fingerprint counts, extracted auth-storage symbol set, absolute `/usr/bin/security` reference, detector-file anchors and persisted-timeout anchors matched. Neither binary was executed; this does not re-establish live store selection or the fallback file path | 1.1.24 (binary read, 2026-09-06; live logs retain their 1.0.10 provenance) |
...
| copilot | per-account tokens coexist in the keychain, so repointing `/lastLoggedInUser` is the entire switch | after a switch, a fresh `copilot -p "say AUTH-OK" --no-color --allow-all-tools` acts as the other account (the cross-account direction is optional — [ACCEPTANCE.md](ACCEPTANCE.md) § Optional account-combination checks) | 1.0.61 |
...
| copilot | **the config directory is a rule**: `--config-dir` (deprecated, hidden) → `COPILOT_HOME` → `~/.copilot`, where `COPILOT_HOME` is the directory *itself* and is used verbatim (no normalization, no absolute-path check). Setting it also skips copilot's one-way migrations into `homedir()/.copilot` — `$XDG_STATE_HOME/.copilot` and `$XDG_CONFIG_HOME/.copilot`, each with its own file list, and each targeting the home directory rather than `COPILOT_HOME`. The auth write goes to `config.json` — **read at 1.0.61 and not re-established since**, because the anchor for it was `writeKey("lastLoggedInUser", …)` in `app.js` and that key name remains absent from 1.0.83's `app.js` (below) — while the bare `config` file in the same directory is a fallback of the settings-migration loader only, behaviour-confirmed again on 1.0.83. What 1.0.83 re-verified about auth is the directory rule, not the file it writes. A **relative** value is resolved against the process's own cwd by both sides, so kae warns rather than pretending to know copilot's | The **installed CLI is not the binary on `PATH`**: that one is a launcher which loads an `app.js` out of the first `pkg` directory it finds, so a version manager can pin an older launcher while `copilot --version` reports the newer package — verify against the version `copilot --version` prints. **Where that `pkg` is has moved**, and reading the old path is how a stale bundle gets measured: at 1.0.79 the launcher builds its search list as `$COPILOT_PKG_CACHE_HOME/pkg`, `$COPILOT_CACHE_HOME/pkg`, `li()/pkg`, `Lr()/copilot/pkg`, `$COPILOT_HOME/pkg`, then `homedir()/.copilot/pkg` **last** — where `li()` is, read from the launcher, `join(homedir(),"Library","Caches","copilot")` on darwin, `(LOCALAPPDATA ?? homedir()/.cache)/copilot` on win32 and `Lr()/copilot` otherwise, and `Lr()` is `XDG_CACHE_HOME ?? homedir()/.cache`. Measured 2026-08-17 on one darwin machine, 1.0.79's package was at `~/Library/Caches/copilot/pkg/darwin-x64/1.0.79/` — a directory of 241 files rather than one `app.js` — while `~/.copilot/pkg/universal/` held only 1.0.36, 1.0.37 and 1.0.61. That is where the artifact table's path resolved until it was repointed on 2026-08-17, and why it answered with an old package; it is an observation of where this build landed, not a rule about where a build must land. Two traps in reading that list, and in stopping the launcher from replacing the package while you read it, are in [measuring.md](../.claude/skills/upstream-auth-drift/references/measuring.md) § Reading a bundle rather than here. **The auth key names are no longer in `app.js`**: `lastLoggedInUser` counts 4 in 1.0.61's `app.js` and **0** in 1.0.79's, `loggedInUsers` 4 and **0**, while over the package directory they are 3 and 4, every hit in `prebuilds/darwin-x64/runtime.node` among serde strings (`internally tagged enum AuthInfo`). So a `lastLoggedInUser` grep of `app.js` now finds nothing and reads as the key being gone; count it over the directory instead (§ Re-measuring's directory form). Two things that is **not** evidence of: that the native module is what *writes* the key, which was not traced, and that the keys moved *because* the package did — 1.0.61 has a `prebuilds` directory too. `grep -o 'COPILOT_HOME[^,;)]\{0,110\}' app.js` still shows the resolver — `t?.configDir ?? process.env.COPILOT_HOME ?? join(homedir(),".copilot")`, also reached through a native `resolveCopilotHome(configDir, COPILOT_HOME, homedir())`, which is the expression kae reproduces — plus the loader's file list (`config.json`, then `config`) and copilot's own help text ("override the directory where configuration and state files are stored"). The migration in the left column is `preAction` running `process.env.COPILOT_HOME \|\| _On()`, where `_On()` calls one migrator per variable, each moving its own file list into `homedir()/.copilot`, leaving a symlink behind and skipping any name already present at the destination. Behavioural confirmation, **on a temp `HOME` and cheaper than the recipe this row used to carry**, which prescribed symlinking `$T/pkg` and passed the real home. Point the *first* search entry at the real package cache and leave everything else temporary: put an `mcp-config` naming a probe server in `$T/cfg`, then `env -i HOME=$T/home PATH=/usr/bin:/bin COPILOT_PKG_CACHE_HOME=~/Library/Caches/copilot COPILOT_AUTO_UPDATE=false COPILOT_HOME=$T/cfg <absolute copilot> mcp list --json` lists it, with nothing written under either home (checked). **`env -i` drops `PATH`, so pass one and spell the binary out** — the claude row above does the same, and this recipe was first recorded without them and did not run. For the relative half, run it from two directories: with `COPILOT_HOME=relhome` the probe is found from the parent of `relhome` and **not** from inside it. **Re-measured 2026-09-05 on 1.0.83** from the darwin-arm64 package: the same two-cwd probe, plus unset, empty, absolute and `--config-dir`-precedence cases, left the resolver and relative-path behaviour unchanged. The settings loader preferred `config.json` over bare `config` and fell back to the latter when the former was absent. The directory-wide `lastLoggedInUser` count remains 1, still in the native auth metadata; that count establishes the anchor remains, not which code writes it. | 1.0.83 (source + behaviour, 2026-09-05) |

The procedures in [ACCEPTANCE.md](ACCEPTANCE.md) are the live-account
re-verification path for these rows. § Real-Machine Acceptance owns the release run;
...
re-measured 2026-09-04 on 2.1.246, several of claude's counts below had moved from
their previously recorded values while the rest held. Which ones moved is
`git log -p` on the table below rather than a figure
...
- `claude.KeychainService` is `"Claude Code-credentials"`, and that string occurs
  **0** times — re-confirmed on 2.1.260, 2026-09-04. Upstream
  builds it as `"Claude Code"` + the OAuth suffix + `"-credentials"`, so only the
  parts are countable: `Claude Code` on its own is noise, while `-credentials` is
...
in every agy build read — 1.0.10, and the one the table now records — even though kae
reads that file for agy's identity — it is a
...
change to nonzero is news worth acting on. agy's `DBUS_*`, `credentials.enc` and
`oauth_creds.json` rows are the same shape, and each is what a doc sentence used to
assert in three places at once: the first pair is why `keyringBypassEnv` gains no
dbus entry, the second is why macOS declares no file artifact.

**`credentials.json`, the remaining `credentialFiles` name, deliberately has no row**,
and why is the rule above working rather than an omission. Its literal count is
**2** on every build read, and both hits are inside
`application_default_credentials.json` and `client_credentials.json` — Google
auth-library noise, not agy naming the file. A row would then fail in both
directions: the noise moves when that library does, and a real occurrence appearing
as one noise hit disappears leaves the count at 2 while the fact the row guards
flips. That is the same ground `antigravity` (175) was rejected on. The standalone
absence is recorded where a reader can act on it, in § Upstream Behaviour
Assumptions.
...
- The **counting** runs only under `mise run audit` (`KAE_FINGERPRINT=1`), which is
  ...
  table records, once per tool, and logs the path it read. The selected command,
  version identity and any required package siblings are established before the
  count begins.

A tool whose selected command or measured artifact cannot be established is a
**failure naming what could not be proved**, never a skip: a fingerprint run that
passes because it found nothing to read would report "the assumptions hold" on no
evidence at all — the failure shape this repo has already shipped twice (a
conformance guard that never examined codex, a doctor check that returned silently).
The version comes from the table, so an upgraded tool lands here, and the remedy is
what an upgrade calls for anyway: re-measure and update both tables. Choosing an
artifact by sorting version-looking directory names was tried and is worse — on one
machine it read copilot 1.0.36 while 1.0.61 was the installed CLI, and mise's
`opencode/1` alias beat `opencode/1.17.4`, each reporting a pile of moved counts for a
...
|---|---|---|
| claude | `realpath(command -v claude)` (one Mach-O, JS inline) | `2.1.261` |
| cursor | `dirname(realpath(command -v cursor-agent))` (webpack chunks; `node` and `index.js` must be present) | `2026.09.02-c22c1a3` |
| copilot | `<normally selected local package>/` from the launcher's documented cache roots and version ordering | `1.0.83` |
| opencode | `realpath(command -v opencode)` (Bun single-file executable) | `1.18.26` |
| agy | `realpath(command -v agy)` (Go binary; SHA-256 `4d1138b2dbde56127969fd307281494d4a7dcc22759ce9adb44d36247df86151`) | `1.1.24` |

The audit machine-checks the recorded version against the command selected by
`PATH`, then derives the artifact through `fingerprintArtifacts` in the test. This
table documents that derivation; the audit logs the resolved path it read, so a
disagreement shows up the first time it runs rather than staying hidden.

These are installed-command observations, not upstream installation contracts. Before
counting, the audit resolves the command selected in its own environment and fails
unless the recorded version, artifact identity, package selection, and required
payload files are all established. It neither executes agy or copilot nor extracts a
missing package; an unmodelled override or ambiguous selection is a failure, not a
guess or skip. The per-tool derivations, source predicates, retained-build traps, and
re-measurement procedure live in
[measuring.md](../.claude/skills/upstream-auth-drift/references/measuring.md) § Reading
a bundle.

**Two of copilot's rows count `changelog.json` as well as code**, which is release
notes and moves when upstream writes about a variable: of `COPILOT_HOME`'s 57, five
are there, and one of `COPILOT_GITHUB_TOKEN`'s 13. That is the noise this section
rejects rows for, and they are kept anyway because narrowing the artifact to one file
is what hid the move in the first place — those literals are spread over multiple
package files. Read a change in those two as "look", not as "the auth code moved".
`lastLoggedInUser`, the key kae actually patches, is confined to the native module and
carries none of it.
...
| Tool | Literal | Count |
|---|---|---|
| claude | `-credentials` | 19 |
| claude | `claude-code-user` | 3 |
| claude | `CLAUDE_CONFIG_DIR` | 59 |
| claude | `CLAUDE_SECURESTORAGE_CONFIG_DIR` | 12 |
| claude | `CLAUDE_CODE_CUSTOM_OAUTH_URL` | 11 |
| claude | `claudeAiOauth` | 28 |
| claude | `oauthAccount` | 77 |
| claude | `refreshTokenExpiresAt` | 12 |
| claude | `profileFetchedAt` | 7 |
| cursor | `-access-token` | 16 |
| cursor | `-refresh-token` | 3 |
| cursor | `-api-key` | 54 |
| cursor | `exchange_user_api_key` | 6 |
| cursor | `partially-authenticated` | 4 |
| cursor | `Logged in as` | 16 |
| copilot | `COPILOT_HOME` | 57 |
| copilot | `lastLoggedInUser` | 1 |
| copilot | `COPILOT_GITHUB_TOKEN` | 13 |
...
| agy | `SSH_TTY` | 1 |
| agy | `SSH_CLIENT` | 1 |
| agy | `DBUS_SESSION_BUS_ADDRESS` | 0 |
| agy | `DBUS_SYSTEM_BUS_ADDRESS` | 0 |
| agy | `credentials.enc` | 0 |
| agy | `oauth_creds.json` | 0 |
