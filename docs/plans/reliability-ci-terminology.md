...
v0.19.0 に認証保全・CI build・用語統一をまとめた。この計画は採否の根拠と
確定方針を残す。認証の保全・修復に必要なリファクタと承認された保全コマンドは
認証テーマに含む。Windows、TUI、対応ツールの拡張は対象外。
...
希望したため、認証テーマを実装した。新しい command・config 契約を含む v0.19.0 の
full gate・リリース検証・Claude 実機受入が通過した。実機ではログインを中断し、
保全・重複抑制・同一コピーの復元・削除確認を検証した。新規ログイン成功や
Codex 実機受入の結果ではない。v0.19.0 の公開と配布物検証も完了した。
結果の正本は [ACCEPTANCE.md](../ACCEPTANCE.md)、公開手順は
[RELEASE.md](../RELEASE.md) に従う。
...
公開・配布物検証・実機受入の結果は [ACCEPTANCE.md](../ACCEPTANCE.md) に記録する。
...
新しい command と設定を追加したため、minor release の v0.19.0 とした。
CI・用語統一の検証結果はその差分の証拠として保持し、認証実装後にも検証と
実機受入を実施した。以前の maintainer-only 適用判断は使用していない。
