...
実装や公開の開始を指示する計画ではなく、次の採用判断と作業順の案である。

## 判断と作業の台帳

| 作業 | 状態 | 採用・完了条件 | 依存 |
|---|---|---|---|
| リリース範囲 | 提案 | 本表の必須・条件付き項目を選択する | ユーザーの計画確認 |
...
| credential の観測・再確認 | 条件付き | 現状維持と局所 module 化を比較する。mapping 再解決・存在・raw bytes 比較の知識が両 caller から減る場合に採用する。既存拒否・実行直前の再確認を両 command で検証する | 範囲確定 |
