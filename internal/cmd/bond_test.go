...
func TestPrepareBondSymlinksNonDenylist(t *testing.T) {
	...
	bondDir, err := app.prepareBond(context.Background(), testBackend(t, app), constants.ToolClaude, "main", pinID, attributionSource{})
	if err != nil {
...
func TestPrepareBondKeepsIdentityPrivateUnderUserConfigDir(t *testing.T) {
	...
	_, stderr = captureStderr(t, func() int {
		dir, err := app.prepareBond(context.Background(), testBackend(t, app),
			constants.ToolClaude, "main", paths.PinID(t.TempDir()), attributionSource{})
		if err != nil {
...
func TestPrepareBondKeepsLinksWhenRealHomeIsUnreadable(t *testing.T) {
	...
	bondDir, err := app.prepareBond(context.Background(), be, constants.ToolClaude, "main", pinID, attributionSource{})
	if err != nil {
	...
	if _, err := app.prepareBond(context.Background(), be, constants.ToolClaude, "main", pinID, attributionSource{}); err != nil {
...
func TestPrepareBondFailsWhenAccountNotCaptured(t *testing.T) {
	...
	_, err := app.prepareBond(context.Background(), testBackend(t, app), constants.ToolClaude, "main", pinID, attributionSource{})
	if err == nil {
