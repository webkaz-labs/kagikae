...
// installMiseGlobalHook registers completion in the shared global fragment,
// migrating an exact older global-config block under the state lock.
func installMiseGlobalHook(env adapter.Env, shell string) (string, bool, error) {
	path, _, _, changed, err := updateGlobalMiseCompletion(env, shell, false)
...
func globalMiseConfigPath(env adapter.Env) string {
	if file := env.Getenv("MISE_GLOBAL_CONFIG_FILE"); file != "" {
		return file
	}
...
// refreshLegacyMiseGlobalHook resumes a pending migration or moves an exact
// generated registration. Customized source blocks are left for manual handling.
func refreshLegacyMiseGlobalHook(env adapter.Env) (string, string, bool, bool, error) {
	return updateGlobalMiseCompletion(env, "", true)
}
