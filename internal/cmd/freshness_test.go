...
func TestSwitchAwayRecapturesRefreshedToken(t *testing.T) {
	...
	// Switch to work, then simulate claude rotating its token in-tool.
	...
	seedClaude(t, app, refreshed, "work-uuid")

	// Switch away: §A must recapture work's live (refreshed) token first.
	...
	if code, out := captureStdout(t, func() int { return runSwitch(ctx, app, opts, "claude", "work") }); code != constants.ExitOK {
		t.Fatalf("switch back to work: %s", out)
	}
...
func TestSwitchAwaySkipsRecaptureWhenMatching(t *testing.T) {
	...
	seedClaude(t, app, workToken, "work-uuid")
	captureStdout(t, func() int { return runCapture(ctx, app, opts, "claude", "work") })
	seedClaude(t, app, personalToken, "personal-uuid")
	captureStdout(t, func() int { return runCapture(ctx, app, opts, "claude", "personal") })

	captureStdout(t, func() int { return runSwitch(ctx, app, opts, "claude", "work") })
	// No in-tool change: live still equals work's snapshot.
