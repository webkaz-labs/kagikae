...
func TestUseIsolatedWritesGlobalFragment(t *testing.T) {
	...
	writeFile(t, realCreds, `{"claudeAiOauth":{"accessToken":"`+personalToken+`"}}`)

	code, out := captureStdout(t, func() int { return runUseIsolated(ctx, app, opts, "claude", "work") })
	mustExit(t, constants.ExitOK, code, out)

	// The global isolated home holds the work credential.
	home := app.Paths.GlobalIsolatedHomeDir(constants.ToolClaude, "work")
	if creds := readFile(t, home+"/.credentials.json"); !strings.Contains(creds, workToken) {
		t.Fatalf("global isolated home must hold the work credential: %s", creds)
	}
	...
	if st.Synced["claude"] != "work" {
		t.Fatalf("state.synced must record claude->work, got %v", st.Synced)
	}
	// The real home is never modified.
	if creds := readFile(t, realCreds); !strings.Contains(creds, personalToken) {
...
func TestUseSharedTearsDownGlobalIsolation(t *testing.T) {
	...
	if creds := readFile(t, app.Env.Home+"/.claude/.credentials.json"); !strings.Contains(creds, workToken) {
...
func TestUseIsolatedUnsupportedTool(t *testing.T) {
	...
	code, out := captureStdout(t, func() int { return runUseIsolated(ctx, app, opts, "agy", "work") })
...
func TestUseIsolatedDryRun(t *testing.T) {
	...
	if _, err := os.Stat(app.Paths.GlobalIsolatedHomeDir(constants.ToolClaude, "work")); !os.IsNotExist(err) {
