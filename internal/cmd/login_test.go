...
// newLoginTestApp seeds a claude main login and replaces the interactive
...
func swapToPersonalToken(t *testing.T) func(string) {
	return func(credsPath string) {
		...
		writeFile(t, credsPath, strings.Replace(live, mainToken, sideToken, 1))
	}
}
...
func TestLoginUnchangedAuthRefused(t *testing.T) {
	...
	code, out := captureStdout(t, func() int { return runLogin(ctx, app, opts, "claude", "main", false) })
	...
	if !strings.Contains(out, "kae add --no-login claude main") {
