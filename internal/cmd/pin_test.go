...
func TestPreparePinConfigSharesOptInItems(t *testing.T) {
	...
	if _, err := app.preparePinConfig(context.Background(), testBackend(t, app),
		constants.ToolClaude, "main", pinID, attributionSource{}); err != nil {
		t.Fatal(err)
	}
...
func TestPreparePinConfigRetractsLinkDroppedFromConfig(t *testing.T) {
	...
	configDir, err := app.preparePinConfig(context.Background(), be, constants.ToolClaude, "main", pinID, attributionSource{})
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := app.preparePinConfig(context.Background(), be, constants.ToolClaude, "main", pinID, attributionSource{}); err != nil {
		t.Fatal(err)
	}
...
func TestPinAndUnpinUsage(t *testing.T) {
	...
	if code := CmdPin(context.Background(), []string{"-i", "claude", "main"}); code != constants.ExitUsage {
...
func TestUseFlagValidation(t *testing.T) {
	...
	if code := CmdUse(ctx, []string{"-s", "-i", "main"}); code != constants.ExitUsage {
...
func TestAddFlagValidation(t *testing.T) {
	...
	if code := CmdAdd(ctx, []string{"claude", "main", "extra"}); code != constants.ExitUsage {
		t.Fatalf("add with three positionals must be a usage error, got %d", code)
	}
	if code := CmdAdd(ctx, []string{"--no-login", "--restore", "claude", "main"}); code != constants.ExitUsage {
		t.Fatalf("--no-login with --restore must be a usage error, got %d", code)
	}
	if code := CmdAdd(ctx, []string{"--dry-run", "claude", "main"}); code != constants.ExitUsage {
