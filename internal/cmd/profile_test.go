...
func TestProfileSaveFromActive(t *testing.T) {
	...
	captureClaude(t, app, "work", workToken) // claude=work active
	...
	if cfg.Profiles["dev"].Accounts["claude"] != "work" {
...
func TestProfileSetRequiresCapturedAccount(t *testing.T) {
	...
	writeConfigFile(t, app, "version = 1\n")
	if _, err := buildProfileSet(ctx, app, commonOpts{Format: formatText}, "dev", "claude", "work"); err != nil {
		t.Fatalf("set: %v", err)
	}
	if reloadConfig(t, app).Profiles["dev"].Accounts["claude"] != "work" {
