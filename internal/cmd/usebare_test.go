...
func TestBareUseQuietSuppressesSuccessOutput(t *testing.T) {
	...
	code, out := captureStdout(t, func() int { return runUseBare(ctx, app, commonOpts{Format: formatText}, false, "work", true) })
	mustExit(t, constants.ExitOK, code, out)
	if out != "" {
