...
func TestEditorPreservesCommentsAndUnrelatedKeys(t *testing.T) {
	...
	if got := cfg.Profiles["work"].Accounts["claude"]; got != "personal" {
		t.Fatalf("claude account not updated: %q", got)
	}
	if cfg.Profiles["work"].Accounts["codex"] != "work" {
		t.Fatalf("sibling key lost: %+v", cfg.Profiles["work"])
	}
}

func TestEditorCreatesProfileAccountTable(t *testing.T) {
	out := edit(t, writerSample, func(e *Editor) {
		e.SetProfileAccount("ci", "claude", "clientA")
	})
	cfg := loadString(t, out)
	if got := cfg.Profiles["ci"].Accounts["claude"]; got != "clientA" {
...
func TestEditorRemoveProfileAccount(t *testing.T) {
	out := edit(t, writerSample, func(e *Editor) {
		if !e.RemoveProfileAccount("work", "codex") {
			t.Fatal("RemoveProfileAccount reported no change")
		}
	})
	cfg := loadString(t, out)
	if _, ok := cfg.Profiles["work"].Accounts["codex"]; ok {
		t.Fatalf("codex mapping not removed: %+v", cfg.Profiles["work"])
	}
	if cfg.Profiles["work"].Accounts["claude"] != "work" {
		t.Fatalf("sibling key lost: %+v", cfg.Profiles["work"])
	}
}
...
func TestEditorRemoveProfile(t *testing.T) {
	...
	out := edit(t, writerSample, func(e *Editor) {
		if !e.RemoveProfile("personal") {
			t.Fatal("RemoveProfile reported no change")
		}
	})
	if strings.Contains(out, "[profiles.personal") {
		t.Fatalf("profile section survived:\n%s", out)
	}
	cfg := loadString(t, out)
	if _, ok := cfg.Profiles["personal"]; ok {
		t.Fatalf("personal profile not removed: %+v", cfg.Profiles)
	}
...
func TestEditorSetAndClearDefaultProfile(t *testing.T) {
	set := edit(t, writerSample, func(e *Editor) { e.SetDefaultProfile("personal") })
	if cfg := loadString(t, set); cfg.DefaultProfile != "personal" {
	...
	noDefault := "version = 1\n[profiles.work.accounts]\nclaude = \"work\"\n"
	added := edit(t, noDefault, func(e *Editor) { e.SetDefaultProfile("work") })
	if cfg := loadString(t, added); cfg.DefaultProfile != "work" {
