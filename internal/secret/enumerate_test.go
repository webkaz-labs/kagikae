...
func TestFileBackendKeys(t *testing.T) {
	...
	want := []string{"backup/20260101/claude/claude_ai_oauth", "claude/work/claude_ai_oauth", "codex/personal/auth"}
	for _, k := range want {
...
func TestLibsecretKeysParsesAttributeLines(t *testing.T) {
	out := "[/org/.../1]\nlabel = kagikae/claude/work/claude_ai_oauth\n" +
		"secret = SECRETVALUE\nattribute.service = kagikae\nattribute.key = claude/work/claude_ai_oauth\n" +
		"\n[/org/.../2]\nattribute.key = codex/personal/auth\n"
	...
	runner.With(fake, func() {
		...
		if len(keys) != 2 || keys[0] != "claude/work/claude_ai_oauth" || keys[1] != "codex/personal/auth" {
