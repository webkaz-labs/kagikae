...
func TestFileBackendRoundTrip(t *testing.T) {
	...
	if _, found, err := b.Get(ctx, "claude/main/oauth"); err != nil || found {
		t.Fatalf("expected absent: found=%v err=%v", found, err)
	}
	payload := []byte(`{"token":"s3cret"}`)
	if err := b.Set(ctx, "claude/main/oauth", payload); err != nil {
		t.Fatal(err)
	}
	got, found, err := b.Get(ctx, "claude/main/oauth")
	if err != nil || !found || string(got) != string(payload) {
		t.Fatalf("round trip failed: %s %v %v", got, found, err)
	}
	path := filepath.Join(b.dir, "claude", "main", "oauth.secret")
	...
	if err := b.Delete(ctx, "claude/main/oauth"); err != nil {
		t.Fatal(err)
	}
	if err := b.Delete(ctx, "claude/main/oauth"); err != nil {
...
func TestKeychainBackend(t *testing.T) {
	...
	runner.With(fake, func() {
		_, found, err := b.Get(ctx, "claude/main/oauth")
		if err != nil || found {
...
func TestLibsecretBackend(t *testing.T) {
	...
	fake := &runnertest.Fake{Stdout: encoded}
	runner.With(fake, func() {
		got, found, err := b.Get(ctx, "codex/main/auth")
		if err != nil || !found || string(got) != string(payload) {
			t.Fatalf("get: %s %v %v", got, found, err)
		}
	})
	...
	runner.With(fake, func() {
		_, found, err := b.Get(ctx, "codex/main/auth")
		if err != nil || found {
			t.Fatalf("expected not found: %v %v", found, err)
		}
	})
	...
	runner.With(fake, func() {
		if _, _, err := b.Get(ctx, "codex/main/auth"); err == nil {
	...
	fake = &runnertest.Fake{}
	runner.With(fake, func() {
		if err := b.Set(ctx, "codex/main/auth", payload); err != nil {
			t.Fatal(err)
		}
	})
