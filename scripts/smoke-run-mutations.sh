...
mutate() {
  ...
  case $id in
    ...
    trailing-temp) subst_once "$runner" 'while [ "${temp_parent%/}" != "$temp_parent" ]; do temp_parent=${temp_parent%/}; done' ':' ;;
    runner-allocation) subst_once "$runner" 'safe=$(mktemp -d "$temp_parent/kae-smoke-run.XXXXXXXX") || exit 2' 'safe=$(mktemp -d "$temp_parent/kae-smoke-run.XXXXXXXX")' ;;
...
full|trailing-temp|trailing TMPDIR separators are normalized in sourced HOME
